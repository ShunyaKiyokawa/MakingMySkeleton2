...
public class MySkeletonController {
	...
	public ModelAndView index(@ModelAttribute("formModel") MyDataEntity mydata, ModelAndView mav) {
		...
		// mav.addObject("radioItems", RADIO_ITEMS); //うまく動かないのでひとまずコメントアウト
	...
	public ModelAndView form(@ModelAttribute("formModel") @Validated MyDataEntity mydata, BindingResult result,
			ModelAndView mov) {
		...
		// mov.addObject("radioItems", RADIO_ITEMS); //なんか動かないのでコメントアウト
		/*
		 * // パスワードを暗号化する BCryptPasswordEncoder encoder = new
		 * BCryptPasswordEncoder(); String passwordHash =
		 * encoder.encode(password.getPasswordHash()); //
	...
	public ModelAndView edit(@ModelAttribute MyDataEntity mydata, @PathVariable int id, ModelAndView mav) {
	...
	@Transactional(readOnly = false)
	public ModelAndView update(@ModelAttribute MyDataEntity mydata, ModelAndView mav) {
		repository.saveAndFlush(mydata);
		mav.addObject("role_user", "ROLE_USER");
	...
	public ModelAndView Ajax(@ModelAttribute("formModel") MyDataEntity mydata, ModelAndView mav) {
	...
	@Transactional(readOnly = false)
	public ModelAndView Ajax(@ModelAttribute("formModel") @Validated MyDataEntity mydata, BindingResult result,
			ModelAndView mov) {
		ModelAndView res = null;
		if (!result.hasErrors()) {
			repository.saveAndFlush(mydata);
	...
	public ModelAndView BootStrap(@ModelAttribute("formModel") MyDataEntity mydata, ModelAndView mav) {
	...
	@Transactional(readOnly = false)
	public ModelAndView BootStrap(@ModelAttribute("formModel") @Validated MyDataEntity mydata, BindingResult result,
			ModelAndView mov) {
		ModelAndView res = null;
		if (!result.hasErrors()) {
			repository.saveAndFlush(mydata);
	...
	public ModelAndView adminindex(@ModelAttribute("formModel") MyDataEntity mydata, ModelAndView mav) {
	...
	@Transactional(readOnly = false)
	public ModelAndView adminform(@ModelAttribute("formModel") @Validated MyDataEntity mydata, BindingResult result,
			ModelAndView mov) {
		ModelAndView res = null;
		if (!result.hasErrors()) {
			repository.saveAndFlush(mydata);
