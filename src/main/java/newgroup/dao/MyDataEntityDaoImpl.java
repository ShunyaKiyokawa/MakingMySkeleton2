...
public class MyDataEntityDaoImpl implements MyDataEntityDaoIF<MyDataEntity> {
	...
	public MyDataEntity findById(long id) {
		return (MyDataEntity) entityManager.createQuery("from MyDataEntity where id = " + id).getSingleResult();
	}

	@Override
	public List<MyDataEntity> findByName(String username) {
		return (List<MyDataEntity>) entityManager.createQuery("from MyDataEntity where username = " + username)
				.getResultList();
	}
	...
	public List<MyDataEntity> find(String fstr) {
		...
		Query query = entityManager.createNamedQuery("findWithName").setParameter("fusername", "%" + fstr + "%");
	...
	/*
	 * @Override public List<MyDataEntity> find(String fstr){ List<MyDataEntity>
	 * list = null; String qstr =
	 * "from MyDataEntity where id = :fid or username like :fusername or mail like :fmail"
	 * ; //idが:fidと一致する場合、またはusernameが:fusernameに似ている場合、またはmailが:fmailに似ている場合
	 * Long fid = 0L; //0LのLはlongを意味する。つけないとintとして扱われ、longで宣言したidで矛盾が生じる try {
	 * fid = Long.parseLong(fstr); } catch (NumberFormatException e) {
	 * //e.printStackTrace(); } Query query = entityManager.createQuery(qstr)
	 * .setParameter("fid", fid) .setParameter("fusername", "%" + fstr + "%")
	 * .setParameter("fmail", fstr + "@%"); list = query.getResultList(); return
	 * list; }
	 */
	@Override
	public List<MyDataEntity> findByAge(int min, int max) {
		return (List<MyDataEntity>) entityManager.createNamedQuery("findByAge").setParameter("min", min)
